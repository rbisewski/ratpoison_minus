...
//! Get the next group from the global list of groups.
/*
 * @return  rp_group*  Pointer to next group from the global list.
 */
rp_group * group_next_group (void)
{
    ...
    //
    if (rp_current_group->node.next == &rp_groups) {
        return list_entry((&rp_groups)->next, typeof(*rp_current_group), node);
    }

    // Otherwise just grab the next pointer contents and return that.
    return list_entry(rp_current_group->node.next, typeof(*rp_current_group), node);
}

//! Return the previous / last group from the globabl list of groups.
/*
 * @returns  rp_group   Previous or last group.
 */
rp_group * group_prev_group (void)
{
  ...
  //
  if (rp_current_group->node.prev == &rp_groups) {
      return list_entry((&rp_groups)->prev, typeof(*rp_current_group), node);
  }

  // Otherwise just pull one off the back. 
  return list_entry(rp_current_group->node.prev, typeof(*rp_current_group), node);
}
...
//! Grab the next window from a group.
/*
 * @param   rp_group*   Group from which to search for the next window.
 * @param   rp_window*  Current window.
 *
 * @return  rp_window*  Pointer to the next window.
 */
rp_window* group_next_window (rp_group *g, rp_window *win)
{
  ...
  // Attempt to grab the window element from the list of mapped windows.
  we = group_find_window (&g->mapped_windows, win);
  ...
  // Attempt to grab the next window entry.
  //
  // i.e. cur = list_next_entry(we, &g->mapped_windows, node);
  //
  if (we->node.next == &g->mapped_windows) {
      cur = list_entry((&g->mapped_windows)->next, typeof(*we), node);
  } else {
      cur = list_entry(we->node.next, typeof(*we), node);
  }
  ...
  while (cur != we) {
      ...
      // If this has an existing entry *and* it's not already present in
      // one or more of the the foreground screens, then return that one.
      if (!find_windows_frame(cur->win)
        && (cur->win->scr == win->scr || rp_have_xinerama)) {
          return cur->win;
      }

      // If not, then move on the next entry in the list.
      //
      // i.e. cur = list_next_entry (cur, &g->mapped_windows, node);
      //
      if (cur->node.next == &g->mapped_windows) {
          cur = list_entry((&g->mapped_windows)->next, typeof(*cur), node);
      } else {
...
//! Return the previous / last window from a group of windows.
/*
 * @param    rp_group*    Group to search thru
 * @param    rp_window*   Current window, needed so as to verify the
 *                        given group is what is expected.
 *
 * @returns  rp_window*   Pointer to the previous / last window.
 */
rp_window * group_prev_window (rp_group *g, rp_window *win)
{
  ...
  // Input validation, if there is no window, then get the last accessed one.
  if (win == NULL) {
      return group_last_window (g, current_screen());
  }
  ...
  // Grab the previous / last window frame.
  //
  // i.e. cur = list_prev_entry(we, &g->mapped_windows, node);
  //
  if (we->node.prev == &g->mapped_windows) {
      cur = list_entry((&g->mapped_windows)->prev, typeof(*we), node);
  } else {
      cur = list_entry(we->node.prev, typeof(*we), node);
  }
  ...
  while (cur != we) {
      ...
      // Can't find the window frame? Probably something odd is going on, or
      // this only has one window. Ergo, just return the current window to
      // be on the safe side.
      if (!find_windows_frame(cur->win)
        && (cur->win->scr == win->scr || rp_have_xinerama)) {
           return cur->win;
      ...
      // Otherwise pop the last one off the back, and try again...
      //
      // i.e. cur = list_prev_entry(cur, &g->mapped_windows, node);
      //
      if (cur->node.prev == &g->mapped_windows) {
          cur = list_entry((&g->mapped_windows)->prev, typeof(*cur), node);
      } else {
