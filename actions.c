  ...
  if (list_size (&rp_frame_undos) > defaults.maxundos)
    {
      ...
      cur = list_entry((&rp_frame_undos)->prev, typeof(*cur), node);
      if (&cur->node == (&rp_frame_undos)) {
          cur = NULL;
      }
...
pop_frame_list (struct list_head *undo_list, struct list_head *redo_list)
{
  ...
  first = list_entry(undo_list->next, typeof(*first), node);
  if (&first->node == undo_list) {
      first = NULL;
  } 

  // Sanity check, make sure this actually grabbed the first element.
  if (!first) {
      return NULL;
  }
...
//! Sync the screen to the current window.
/*
 * @param   rp_screen*  Pointer to the screen to sync.
 *
 * @return  none 
 */
static void sync_wins (rp_screen *s)
{
  ...
  // Take the display, assemble a Xwin query tree.
  XQueryTree(dpy, s->root, &dw1, &dw2, &wins, &nwins);
  
  // Attempt to grab the next position and entry.
  win    = list_entry((&rp_mapped_window)->next, typeof(*win), node);
  wintmp = list_entry(win->node.next, typeof(*win), node);

  // Remove any windows in our cached lists that aren't in the query
  // tree. These windows have been destroyed.
  //
  // i.e. list_for_each_entry_safe (win, wintmp, &rp_mapped_window, node)
  //
  while (&win->node != &rp_mapped_window) {

      // Variable to track whether or not the window was found.
      int found = 0;

      // Got thru all the windows, looking for a match.
      for (i = 0; i < nwins; i++) {
      ...
      // Since this found something, get the next window.
      if (found) {

          // Set the position, then grab the next entry.
          win    = wintmp;
          wintmp = list_entry(win->node.next, typeof(*win), node);

          // Use the while-statement to check if the node is what is wanted. 
          continue;
      }

      // Otherwise the search found nothing, so increment the ignored
      // bad-window counter.
      ignore_badwindow++;
      ...
      // unmanaged.
      //
      // A) In an iconic state? Simply withdraw it. 
      //
      ...
      //
      // B) In a noraml state? Gotta cleanup the frame, then withdraw it.
      //
      } else if (win->state == NormalState) {

          // If debug, tell the end user what happened.
          PRINT_DEBUG (("Destroying Normal Window (%s)\n", window_name (win)));

          // Attempt to grab the frame.
          rp_frame *frame = find_windows_frame(win);

          // If this gotta frame, then clean 'er.
          if (frame) {
      ...
      // With all of that done, set the position, then grab the next entry
      // so as to determine if it is the intended window.
      win    = wintmp;
      wintmp = list_entry(win->node.next, typeof(*win), node);
  }

  // Iterate thru every window...
  for (i = 0; i < nwins; i++) {

      // Grab the attributes
      XGetWindowAttributes(dpy, wins[i], &attr);

      // Just a plain ol' window?
      ...
      if (win) {
          ...
          // Not viewable or not unmapped? The window has probably been
          // freed or was never assigned. Ergo, skip this window.
          if (attr.map_state != IsViewable || attr.map_state != IsUnmapped) {
              PRINT_DEBUG(("Note: Nonstandard or nonexistent window map "
                           "state detected!\n"));
              continue;
          }

          // Attempt to grab the window frame.
          rp_frame *frame = find_windows_frame(win);

          // Viewable via the frame? Maximize it!
          if (attr.map_state == IsViewable && frame) {
              maximize(win);

          // Viewable but no frame? Hide it!
          } else if (attr.map_state == IsViewable) {
              hide_window(win);

          // Unmapped and iconic and frame? Unhide and maximize it!
          } else if (attr.map_state == IsUnmapped
            && get_state(win) == IconicState && frame) {
              unhide_window (win);
              maximize (win);
          }

          // All done with this window here, so go to the next window.
          continue;
      }

      // Attempt to grab the unmapped window.
      win = find_window_in_list (wins[i], &rp_unmapped_window);

      // Got an unmapped window?
      if (win) {

          // Not viewable or not unmapped? The window has probably been
          // sync'd or was never assigned. Ergo, skip this window.
          if (attr.map_state != IsViewable || attr.map_state != IsUnmapped) {
              PRINT_DEBUG(("Note: Pre-existing sync'd window detected!\n"));
              continue;
          }
          ...
          // Unmapped and iconic? Then map and hide it!
          } else if (attr.map_state == IsUnmapped
      ...
      // The window isn't in the mapped or unmapped window list so add it/
      win = add_to_window_list (s, wins[i]);

      // If it's visible or iconized then map it. 
      if (attr.map_state == IsViewable || (attr.map_state == IsUnmapped
        && get_state (win) == IconicState)) {
          map_window (win);
      }
    ...
    // At this point, simply leave since all the windows have been sync'd.
    return;
}
  ...
  if (args[0] == NULL)
    {
      ...
      last = list_entry((&set_vars)->prev, typeof(*last), node);
      if (&last->node == (&set_vars)) {
          last = NULL;
      }
