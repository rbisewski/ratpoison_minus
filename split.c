...
//! Finds and returns a pointer to the next frame.
/*
 * @param   rp_frame*  Current frame.
 *
 * @return  rp_frame*  Pointer to the next frame.
 */
rp_frame* find_frame_next(rp_frame *frame)
{
    // Input validation, make sure this got a frame.
    if (frame == NULL) {
        return NULL; 
    }
    ...
    //
    if (frame->node.next == (&frames_screen (frame)->frames)) {
        return list_entry((&frames_screen (frame)->frames)->next,
                          typeof(*frame),
                          node);
    }

    // Otherwise just grab the pointer to the next node and return that. 
    return list_entry(frame->node.next, typeof(*frame), node);
}

//! Find and grab the previous / last frame
/*
 * @param   rp_frame   Current frame, to search thru.
 *
 * @return  rp_frame   Pointer to the previous / last frame.
 */
rp_frame * find_frame_prev (rp_frame *frame)
{
  ...
  //
  if (frame->node.prev == (&frames_screen (frame)->frames)) {
      return list_entry((&frames_screen (frame)->frames)->prev,
                        typeof(*frame),
                        node);
  }

  // Otherwise just pull one off the back.
  return list_entry(frame->node.prev, typeof(*frame), node);
}
